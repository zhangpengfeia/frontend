...
1. 前端 JS 加载 HLS.js
2. HLS.js 初始化，绑定 <video> 标签
3. HLS.js 下载 .m3u8 播放列表
4. HLS.js 解析播放列表，得到每个切片 URL
5. HLS.js 按顺序下载 TS/fMP4 切片
6. HLS.js 用 MSE 的 SourceBuffer.appendBuffer() 将切片数据送入 <video>
7. <video> 内部解码器解码并渲染
8. HLS.js 管理缓冲、丢包、码率切换
```

## WebRTC(重点)
WebRTC（Web Real-Time Communication） 是一套由浏览器原生支持的实时音视频通信技术标准，它允许浏览器之间直接传输：
- 音频（Audio）
- 视频（Video）
- 任意数据（DataChannel）
最重要的是：**不需要安装任何插件**！
**WebRTC不是流媒体协议，而是实时通信框架**，主要解决**超低延迟的双向音视频通信**
```typescript
// 核心对比
HLS/DASH: 服务器 → 广播给很多人 (一对多)
WebRTC:   你 ↔ 我 || 你 ↔ 服务器 ↔ 我 (双向/多向)
```